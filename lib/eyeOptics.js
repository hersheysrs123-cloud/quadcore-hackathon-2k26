...
// ─── Refractive error ───────────────────────────────────────────────
// Myopia and hypermetropia, as a single signed number.
//
// `errorD` is the spectacle prescription, in dioptres, in the usual
// convention: NEGATIVE for short sight (a diverging lens corrects it) and
// POSITIVE for long sight. Zero is an emmetropic eye and every function
// below reduces to what it did before.
//
// The model is REFRACTIVE ametropia — a normal-length globe whose optics are
// too strong or too weak — rather than axial. Both exist; this one is chosen
// because the axial version moves the retina, and the retina is drawn. A
// panel describing a 23.4 mm globe over a scene drawing a 22.2 mm one is the
// exact defect this whole audit was about, and the teaching behaviour is
// identical either way: the rays cross in front of the retina or behind it.

/** How far the eye's relaxed power is from the 60 D it needs, by defect. */
export const DEFECTS = {
  myopia: {
    key: "myopia",
    label: "Short sight (myopia)",
    lens: "diverging (concave)",
    sign: -1,
    blurs: "distant objects",
    cause: "the eye refracts too strongly, so parallel rays cross IN FRONT of the retina",
  },
  none: {
    key: "none",
    label: "Normal (emmetropic)",
    lens: "none needed",
    sign: 0,
    blurs: "nothing — the whole range from the near point to infinity is available",
    cause: "the relaxed eye focuses parallel rays exactly on the retina",
  },
  hypermetropia: {
    key: "hypermetropia",
    label: "Long sight (hypermetropia)",
    lens: "converging (convex)",
    sign: 1,
    blurs: "near objects",
    cause: "the eye refracts too weakly, so rays would cross BEHIND the retina and it must accommodate even for distance",
  },
};

/** Which defect a prescription describes. Below a sixth of a dioptre is nothing. */
export function defectFor(errorD) {
  const e = Number(errorD) || 0;
  if (e < -0.16) return DEFECTS.myopia;
  if (e > 0.16) return DEFECTS.hypermetropia;
  return DEFECTS.none;
}

/**
 * The relaxed power this eye actually has.
 *
 * A myope's prescription is negative and their eye is too STRONG, so the sign
 * flips: −3.00 D of correction means 63 D of eye.
 */
export const relaxedPowerWith = (errorD) => EYE.relaxedPower - (Number(errorD) || 0);

/** Power the correcting lens supplies when it is being worn. */
export const correctionPower = (errorD, corrected) => (corrected ? Number(errorD) || 0 : 0);

/**
 * Far point: the furthest object still focusable, metres.
 *
 * Only a myope has a finite one — beyond it the eye cannot relax any further,
 * because relaxing is all it has. A hypermetrope reaches infinity by
 * accommodating, which is why long sight is missed for so long.
 */
export function farPoint(errorD, corrected = false) {
  const e = (Number(errorD) || 0) - correctionPower(errorD, corrected);
  return e < -1e-9 ? 1 / -e : Infinity;
}

/** Near point for this eye, metres — the amplitude has to cover the error too. */
export function nearPointFor(errorD, corrected = false) {
  const e = (Number(errorD) || 0) - correctionPower(errorD, corrected);
  const available = EYE.accommodationAmplitude - e;
  return available > 1e-9 ? 1 / available : Infinity;
}
...
/**
 * Accommodation this eye must supply for an object at `objectDistanceM`.
 *
 * `required − what the eye already has`, which works out as `errorD + 1/u`
 * once the correction is taken off. A myope's is NEGATIVE at distance: the
 * eye has too much power and cannot take any away, which is precisely why
 * distance is the thing they cannot fix.
 */
export function accommodationDemandWith(objectDistanceM, errorD = 0, corrected = false) {
  const e = (Number(errorD) || 0) - correctionPower(errorD, corrected);
  return e + 1 / Math.max(objectDistanceM, 1e-6);
}
 ...
 * hard you try.
 */
export function appliedAccommodation(
  objectDistanceM,
  { auto = true, manual = 0, errorD = 0, corrected = false } = {},
) {
  const demand = accommodationDemandWith(objectDistanceM, errorD, corrected);
  const supplied = auto ? demand : manual;
  // The ciliary muscle can only ADD power. It cannot take any away, which is
  // the whole of short sight.
...
export function solveEye({
  ...
  const errorD = Number(refractiveErrorD) || 0;
  const defect = defectFor(errorD);
  const correction = correctionPower(errorD, corrected);
  const wearing = corrected && defect.key !== "none";

  const accommodation = appliedAccommodation(objectDistanceM, {
    auto,
    manual: manualAccommodation,
    errorD,
    corrected,
  });
  // The eye's own relaxed power, plus the spectacle lens if it is being worn,
  // plus whatever the ciliary muscle is adding.
  const power = relaxedPowerWith(errorD) + correction + accommodation;
  ...
  return {
    refractiveErrorD: errorD,
    defect,
    corrected: wearing,
    correctionD: correction,
    relaxedPower: relaxedPowerWith(errorD),
    farPoint: farPoint(errorD, corrected),
    nearPoint: nearPointFor(errorD, corrected),
    ...
    /**
     * Past the far point the eye has run out of RELAXATION, not of lens.
     * Only a myopic eye can be here, and no amount of effort helps.
     */
    beyondFarPoint: demand < -1e-9,
    power,
