...
/**
 * How far either end of a lever may swing below its pivot, metres. The pivot
 * stands a fixed height above the bench, and this is what is left of that
 * height once a margin is kept clear of the bench top.
 */
export const MAX_END_DROP_M = 0.476;

/** The steepest a lever is allowed to tilt, as the sine of its angle (≈ 33° / 20°). */
const MAX_SIN_CLASS_1 = 0.55;
const MAX_SIN_OTHER = 0.35;

/**
 * How far the load is actually raised in one stroke, metres.
 *
 * LIFT_M for the tackle. For a lever it is the stroke that fits: the bar has to
 * stay clear of the bench at its low end and not tilt absurdly, so a lever
 * with a short load arm cannot lift 25 cm — it would have to swing past
 * vertical. This used to be worked out inside the scene while the Details
 * panel assumed 25 cm regardless, so the two printed different "load moves",
 * "work in" and "work out" for the same machine.
 */
export function strokeLift(type, p) {
  if (type === "pulley") return LIFT_M;
  const { loadArm, effortArm } = leverLayout(type, p);
  const sinMax =
    type === "lever1"
      ? Math.min(Math.max(MAX_END_DROP_M / Math.max(effortArm, 1e-6), 0.05), MAX_SIN_CLASS_1)
      : MAX_SIN_OTHER;
  return Math.min(LIFT_M, loadArm * sinMax);
}
 ...
 */
export function solveMachine({ type = "lever1", p = 0.35, sheaves = 2, loadN = 300, lift = strokeLift(type, p) }) {
