 ...
 * state, custom spaces, and IndexedDB spaceSettings.
 */
export async function getSavedSpaces() {
  ...
  try {
    ...
    // 1. Initialize with standard SPACES
    SPACES.forEach((s) => merged.set(s.name, { ...s }));

    // 2. Overlay any saved spaces from localStorage (which has user custom emojis/names)
    ...
    // 4. Overlay settings from IndexedDB db.spaceSettings (e.g. icon/emoji)
    Object.entries(dbSettingsMap).forEach(([spName, s]) => {
      if (spName && (s.icon || s.blurb)) {
        const base = merged.get(spName) || { name: spName };
        merged.set(spName, {
          ...base,
          ...(s.icon ? { icon: s.icon } : {}),
