...
} from "./noteHierarchy.js";
import { resetLiteratureData } from "./literatureService.js";
...
export async function saveNote(noteData) {
  ...
  // Nested sub-pages: `undefined` keeps whatever parent the stored note already
  // has (partial saves from the editor never carry it), `null` explicitly makes
  // the note top-level. A note can never be its own parent.
  let parentId =
    noteData.parentId !== undefined
      ? normalizeParentId(noteData.parentId)
      : normalizeParentId(existingNote?.parentId);
...
function addDeletedNoteTombstones(ids = []) {
  try {
    const deleted = JSON.parse(localStorage.getItem("socratic_deleted_notes") || "[]");
    ...
    if (changed) localStorage.setItem("socratic_deleted_notes", JSON.stringify(deleted));
  } catch (e) {}
}

function removeDeletedNoteTombstones(ids = []) {
  try {
    const deleted = JSON.parse(localStorage.getItem("socratic_deleted_notes") || "[]");
    if (!Array.isArray(deleted)) return;
    const removeSet = new Set(ids);
    const updated = deleted.filter((d) => !removeSet.has(d));
    if (updated.length !== deleted.length) {
      localStorage.setItem("socratic_deleted_notes", JSON.stringify(updated));
    }
  } catch (e) {}
}

/**
 * Moves a note AND every nested sub-page below it into the trash as one unit
 * (they share the same `deletedAt`, so they expire and purge together).
 * Returns the trashed note ids, root first.
 */
export async function deleteNoteToTrash(id) {
  ...
  await db.transaction("rw", [db.notes, db.trash], async () => {
    ...
    const descendantIds = collectDescendantIds(allNotes, id);
    const subtree = [note, ...descendantIds.map((did) => allNotes.find((n) => n.id === did)).filter(Boolean)];
    const deletedAt = new Date().toISOString();

    await db.trash.bulkPut(subtree.map((n) => ({ ...n, deletedAt })));
    await db.notes.bulkDelete(subtree.map((n) => n.id));
    trashedIds.push(...subtree.map((n) => n.id));
  });

  addDeletedNoteTombstones(trashedIds);
  return trashedIds;
}
...
/**
 * Restores a trashed note together with every sub-page that was trashed
 * beneath it. If the restored root's own parent is no longer live (still in
 * the trash or permanently gone) it is promoted to a top-level note so it can
 * never become an invisible orphan. Returns the restored note objects, root first.
 */
export async function recoverNote(id) {
  ...
  await db.transaction("rw", [db.notes, db.trash], async () => {
    ...
    const descendantIds = collectDescendantIds(allTrash, id);
    const subtree = [trashedItem, ...descendantIds.map((did) => allTrash.find((t) => t.id === did)).filter(Boolean)];

    const liveIds = new Set((await db.notes.toArray()).map((n) => n.id));
    for (const n of subtree) liveIds.add(n.id);

    const now = new Date().toISOString();
    const toRestore = subtree.map((item) => {
      const { deletedAt, ...restoredNote } = item;
      return {
        ...restoredNote,
        parentId: resolveRestoredParentId(restoredNote, liveIds),
        updatedAt: now,
      };
    });

    await db.notes.bulkPut(toRestore);
    await db.trash.bulkDelete(toRestore.map((n) => n.id));
    restored.push(...toRestore);
  });

  removeDeletedNoteTombstones(restored.map((n) => n.id));
  return restored;
}

/** Permanently erases a trashed note and every sub-page trashed beneath it. Returns the erased ids. */
export async function permanentlyDeleteNote(id) {
  ...
  await db.transaction("rw", [db.notes, db.trash], async () => {
    const allTrash = await db.trash.toArray();
    const allNotes = await db.notes.toArray();
    for (const did of collectDescendantIds([...allTrash, ...allNotes], id)) {
      if (!erased.includes(did)) erased.push(did);
    }
    await db.trash.bulkDelete(erased);
    await db.notes.bulkDelete(erased);
  });

  addDeletedNoteTombstones(erased);
  return erased;
}

/**
 * Moves a note and its entire subtree into another space (sub-pages always
 * travel with their parent). The root lands at the end of the target space;
 * descendants keep their relative order. Returns the updated notes, root first.
 */
export async function moveNoteTreeToSpace(rootId, targetSpace, { rootOrder } = {}) {
  if (typeof window === "undefined" || !rootId || !targetSpace) return [];
  ...
  await db.transaction("rw", [db.notes], async () => {
    ...
    if (!root) return;
    const descendantIds = collectDescendantIds(allNotes, rootId);
    const finalRootOrder =
      typeof rootOrder === "number"
        ? rootOrder
        : allNotes.filter((n) => n.id !== rootId && (n.spaceId || n.space) === targetSpace && !normalizeParentId(n.parentId)).length;
    const now = new Date().toISOString();

    const updates = [
      { ...root, spaceId: targetSpace, space: targetSpace, parentId: null, order: finalRootOrder, updatedAt: now },
      ...descendantIds
        .map((did) => allNotes.find((n) => n.id === did))
        .filter(Boolean)
        .map((n) => ({ ...n, spaceId: targetSpace, space: targetSpace, updatedAt: now })),
    ];
    await db.notes.bulkPut(updates);
    moved.push(...updates);
  });
...
/** Direct children of a note, sorted by order. */
export async function getChildNotesOf(parentId) {
  if (typeof window === "undefined" || !parentId) return [];
  const all = await db.notes.toArray();
  return all
    .filter((n) => normalizeParentId(n.parentId) === parentId)
    .sort(
      (a, b) =>
        (a.order ?? 0) - (b.order ?? 0) ||
        (a.createdAt || "").localeCompare(b.createdAt || "") ||
...
export async function resetNotesData() {
  ...
  localStorage.removeItem("socratic_deleted_notes");
...
export async function seedDemoContent({ overwrite = false } = {}) {
  ...
  // Explicit user-invoked seed clears note deletion tombstones so sample notes restore cleanly
  localStorage.removeItem("socratic_deleted_notes");
...
export async function factoryResetWorkspace(target) {
  ...
  if (target === "literature") {
    return await resetLiteratureData();
  }
  ...
  if (db.poems) tables.push(db.poems);
  ...
  await db.transaction("rw", tables, async () => {
    ...
    if (db.poems) await db.poems.clear();
  });
...
export async function renameSpace(oldSpaceName, newSpaceName, newIcon, newBlurb) {
  ...
  try {
    ...
    // 7. Update Literature poems
    if (db?.poems) {
      const poemsToUpdate = await db.poems
        .filter((p) => p.spaceId === oldSpaceName)
        .toArray();
      for (const poem of poemsToUpdate) {
        await db.poems.put({ ...poem, spaceId: trimmedNew, updatedAt: new Date().toISOString() });
      }
    }

    // 8. Update study sessions
    if (db?.studySessions) {
...
export async function deleteSpace(spaceName) {
  ...
  try {
    ...
    // 4. Update socratic_last_workspace_state if it pointed to the deleted space
    try {
      const lastState = JSON.parse(localStorage.getItem("socratic_last_workspace_state") || "null");
      if (lastState && lastState.activeSpace === spaceName) {
        lastState.activeSpace = null;
        lastState.activeNoteId = null;
        localStorage.setItem("socratic_last_workspace_state", JSON.stringify(lastState));
      }
    } catch (e) {}

    // 5. Clean up db.notes in this space (move to trash and mark as deleted notes)
    if (db?.notes && db?.trash) {
      const allNotes = await db.notes.toArray();
      const spaceNotes = allNotes.filter((n) => (n.spaceId || n.space) === spaceName);
      if (spaceNotes.length > 0) {
        ...
        try {
          const delNotes = JSON.parse(localStorage.getItem("socratic_deleted_notes") || "[]");
          const delNotesSet = new Set(delNotes);
          spaceNotes.forEach((n) => delNotesSet.add(n.id));
          localStorage.setItem("socratic_deleted_notes", JSON.stringify(Array.from(delNotesSet)));
        } catch (e) {}
      }
    }

    // 6. Remove from IndexedDB db.spaceSettings
    if (db?.spaceSettings) {
      await db.spaceSettings.delete(spaceName);
    }

    // 7. Remove associated documents from IndexedDB db.spaceDocuments
    if (db?.spaceDocuments) {
    ...
    // 8. Clean up db.folders and db.bookmarks
    if (db?.folders) {
    ...
    // 9b. Clean up Literature poems belonging to this space
    if (db?.poems) {
      const poemRows = await db.poems.where("spaceId").equals(spaceName).toArray();
      if (poemRows.length > 0) {
        await db.poems.bulkDelete(poemRows.map((p) => p.id));
      }
    }

    // 9. Clean up db.quizzes
    if (db?.quizzes) {
...
/**
 * Extract clean text from an uploaded syllabus/curriculum file (.docx, .pdf, or plain text / markdown).
 * Dynamically imports mammoth or pdfjs-dist only when needed.
 *
 * @param {File|Blob} file The uploaded file object
 * @returns {Promise<string>} Cleaned text extracted from the file
 */
export async function extractSyllabusTextFromFile(file) {
  if (!file) return "";
  let text = "";
