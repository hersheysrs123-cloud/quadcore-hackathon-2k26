  ...
  const result = blocks
    .map((block) => {
      ...
      switch (block.type) {
        ...
        case "page":
          return `[sub-page] ${block.emoji ? `${block.emoji} ` : ""}${block.title || content || "Untitled Note"}`;
        default:
          return content;
      }
    })
...
/**
 * Relevance score for a slash-menu query (lower = better, -1 = no match).
 * Exact / prefix matches on the label, type or a keyword outrank the loose
 * substring matches, so "/page" lands on Page (not Text, whose keyword "p"
 * is a substring of the query) and "/h1" lands on Heading 1.
 */
export function scoreSlashItem(item, q) {
  const label = item.label.toLowerCase();
  const type = item.type.toLowerCase();
  const keywords = item.keywords || [];
  if (type === q || label === q) return 0;
  if (label.startsWith(q) || type.startsWith(q)) return 1;
  if (keywords.includes(q)) return 2;
  if (keywords.some((k) => k.startsWith(q))) return 3;
  if (label.includes(q) || type.includes(q)) return 4;
  if (keywords.some((k) => k.includes(q))) return 5;
  if (keywords.some((k) => q.includes(k))) return 6;
  return -1;
}

export function rankSlashItems(items, query) {
  const q = String(query || "").trim().toLowerCase();
  if (!q) return items;
  return items
    .map((item, idx) => ({ item, idx, score: scoreSlashItem(item, q) }))
    .filter((x) => x.score >= 0)
