...
/**
 * The ironbow ramp a thermal imager uses: cold is periwinkle, then violet,
 * red, orange, and finally white-hot.
 *
 * A lighter cut of the classic: a real imager starts at black, but black on a
 * bright bench reads as a hole in the scene. Indigo keeps the property that
 * matters - luminance rises with every step - while staying visibly a colour.
 *
 ...
 */
const IRONBOW = ["#5a6cd8", "#8a5fd0", "#d85f9a", "#f5804a", "#fbb636", "#ffe07a", "#fff9e0"];
...
/**
 * One frame of the beaker's bulk temperature, stepped from where it is.
 *
 * Capped at the boil HERE, in the state itself, and not only when it is read.
 * At full gas the water is being pushed toward 137 °C; the thermometer stops at
 * 100 because past that the energy goes into steam. If the cap lived only in
 * the display, the hidden number would carry on up to 137 and, when the gas was
 * turned down, the beaker would sit at 100 °C for the best part of a minute
 * while that phantom overshoot drained away - the one place in the scene where
 * turning the flame down would visibly do nothing.
 */
export function stepWaterMean(meanC, intensity, dtSeconds, ambientC = AMBIENT_C) {
  const next = relax(meanC, waterAsymptote(intensity, ambientC), WATER_TIME_CONSTANT, dtSeconds);
  return Math.min(next, BOILING_C);
}
...
/**
 * A polished, silvered plate. The same size and the same distance as the black
 * one, and the only difference is what its surface does to radiation: it
 * reflects nearly all of what arrives, so it absorbs almost none of it.
 */
export const SHINY_EMISSIVITY = 0.08;
...
export const absorbedPower = (intensity, distance = PLATE_DISTANCE, emissivity = PLATE_EMISSIVITY) =>
  emissivity * irradianceAt(intensity, distance) * PLATE_AREA;
 ...
 */
export function plateEquilibrium(
  intensity,
  distance = PLATE_DISTANCE,
  ambientC = AMBIENT_C,
  emissivity = PLATE_EMISSIVITY,
) {
  const gain = absorbedPower(intensity, distance, emissivity);
  ...
  const loss = (tC) => {
    const tK = kelvin(tC);
    return (
      emissivity * STEFAN_BOLTZMANN * bothFaces * (Math.pow(tK, 4) - Math.pow(ambientK, 4)) +
 ...
 */
export function plateTimeConstant(workingC = AMBIENT_C, emissivity = PLATE_EMISSIVITY) {
  const tK = kelvin(workingC);
  const slope =
    4 * emissivity * STEFAN_BOLTZMANN * 2 * PLATE_AREA * Math.pow(tK, 3) +
...
export function plateTemperature({
  ...
  const target = plateEquilibrium(intensity, distance, ambientC, emissivity);
  if (target <= ambientC + 1e-9) return ambientC;
  const elapsed = Math.max(seconds, 0) * TIME_LAPSE;
  const approach = Number.isFinite(elapsed)
    ? 1 - Math.exp(-elapsed / plateTimeConstant(target, emissivity))
    : 1;
...
export function solveHeatTransfer({
  ...
  const shinyC = plateTemperature({ intensity, seconds, ambientC, emissivity: SHINY_EMISSIVITY });
  ...
  return {
    ...
    /** The silvered twin of the black plate, same size, same distance. */
    shinyC,
    shinyAbsorbedPower: absorbedPower(intensity, PLATE_DISTANCE, SHINY_EMISSIVITY),
    shinyEquilibriumC: plateEquilibrium(intensity, PLATE_DISTANCE, ambientC, SHINY_EMISSIVITY),

    /** Ceiling for the FLIR ramp, so the false colour spans what is on screen. */
    hottestC: Math.max(water.bottom, selected.tipC, plateC, shinyC, ambientC + 1),
  };
}
