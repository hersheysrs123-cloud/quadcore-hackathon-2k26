 ...
 * the lesson honest: each interrupter removes exactly one side.
 */
export function triangleStatus(state) {
  return {
    fuel: state.fuelOpen,
    oxygen: state.extinguishedBy === "oxygen" ? false : state.jarO2 >= O2_EXTINCTION,
    heat: state.lit || (state.extinguishedBy !== "heat" && (state.extinguishedBy !== null || state.heat >= HEAT_EXTINCTION)),
  };
}
