...
export function normalizeQuiz(raw, fallbackTopic) {
  const validTypes = [
    "multiple_choice",
    "multi_select",
    "short_answer",
...
export function buildQuizTranscript(questions, graded) {
  return questions
    .map((question, i) => {
...
export const clampScore = (n) => Math.max(0, Math.min(100, Math.round(Number(n) || 0)));
