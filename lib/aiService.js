  ...
  const questions = (Array.isArray(raw?.questions) ? raw.questions : [])
    .map((q, i) => {
      ...
      return {
        ...
        correctIndex: finalType === "multiple_choice" && Number.isInteger(index) && index >= 0 && index < options.length ? index : -1,
...
export async function quizGenerate({
  ...
  const distributionPrompt = countsSpecified
    ? `Generate exactly ${numMCQ} 'multiple_choice', ${numShort} 'short_answer', and ${numLong} 'long_answer' questions.`
    : `Generate exactly 5 'multiple_choice' and 3 'short_answer' questions. Total 8 questions.`;
...
${personaPrompt}
${strictnessPrompt}
${effectiveLevel && effectiveLevel !== "general" ? `Academic Grade Level: ${effectiveLevel.toUpperCase()}` : ""}
${difficultyPrompt}
${distributionPrompt}
${focusPrompt}
${syllabusPrompt}
