...
export function silhouette(shape, rotationRad = 0, size = 10) {
  ...
  switch (shape) {
    ...
    case "cone": {
      // Rotating vertically toward the screen: standing upright it blocks a
      // triangle; turned end-on, its circular base faces the screen.
      const halfWidth = r * 0.62;
      const halfHeight = r * (1.1 * c + 0.62 * s);
      return {
        halfWidth,
        halfHeight,
        kind: s > 0.94 ? "circle" : c > 0.94 ? "triangle" : "conical",
        description:
          s > 0.94
            ? "a circle — looking straight into the base of the cone"
            : c > 0.94
              ? "a triangle — the cone standing upright"
              : "a tapered conical silhouette as the cone tilts",
        tiltProgress: s,
      };
    }
    case "sphere":
    ...
    case "ring": {
      // A torus ring facing the light blocks a ring with an open hole.
      // Turned 90° edge-on, it blocks a solid narrow bar.
      const halfWidth = r * 0.8;
      const halfHeight = r * (0.8 * c + 0.22 * s);
      return {
        halfWidth,
        halfHeight,
        kind: s > 0.88 ? "capsule" : "ring",
        description:
          s > 0.88
            ? "a narrow bar — the ring turned edge-on"
            : s > 0.15
              ? "an oval ring with an oval opening in the middle"
              : "a circular ring with a round hole in the middle",
        tiltProgress: s,
      };
    }
    case "pyramid": {
      // A 4-sided square pyramid: standing upright it blocks a triangle;
      // tilted 90° vertically, its flat square base faces the screen.
      const halfWidth = r * 0.75;
      const halfHeight = r * (1.0 * c + 0.75 * s);
      return {
        halfWidth,
        halfHeight,
        kind: s > 0.94 ? "rectangle" : c > 0.94 ? "triangle" : "pyramid",
        description:
          s > 0.94
            ? "a square — looking straight into the base of the pyramid"
            : c > 0.94
              ? "a triangle — the pyramid standing upright"
              : "a trapezoid profile as the pyramid tilts",
        tiltProgress: s,
      };
    }
    case "letterT": {
      const rawCos = Math.cos(rotationRad);
      const cosVal = Math.abs(rawCos) < 1e-12 ? 0 : rawCos;
      return {
    ...
    case "letterL": {
      ...
      return {
        ...
        description:
          c > 0.7
            ? cosVal < -1e-9
              ? "the reversed letter L (seen from the back)"
              : "the letter L"
            : "a narrow bar — the L turned edge-on",
      };
    }
    default:
...
export const SHAPES = ["cylinder", "cube", "cone", "sphere", "ring", "pyramid", "letterT", "letterL"];
