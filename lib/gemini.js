...
export async function getEffectiveProvider() {
  if (typeof window !== "undefined") {
    try {
      ...
      if (providerItem?.value) return providerItem.value;
    } catch {
      // Ignore
    }
  }
...
export async function generate({
  ...
  if (provider === "webllm" && typeof window !== "undefined") {
    const { generateWebLLM } = await import("./webllm");
    return generateWebLLM({ system, messages, temperature, schema });
  }
  ...
  if (!response.ok) {
    ...
    if ((response.status === 400 || response.status === 403) && /API key|invalid/i.test(message)) {
