...
/**
 * Single seam for model choice. Standard Google AI Studio models:
...
export async function getEffectiveApiKey() {
  if (typeof window !== "undefined") {
    try {
      ...
      if (keyItem?.value && keyItem.value.trim()) return keyItem.value.trim();
    } catch {
...
export async function generate({
  ...
  if (!apiKey) throw new GeminiError("Gemini API key is not configured in settings or environment.", 500);
  ...
  if (!response.ok) {
    ...
    if ((response.status === 400 || response.status === 401 || response.status === 403) && /API_KEY_INVALID|API key not valid|API key is invalid|API key expired|UNAUTHENTICATED|PERMISSION_DENIED/i.test(message)) {
      throw new GeminiError("Gemini API key is invalid or unauthorized. Please check your API key in Settings ⚙️ or .env.local.", response.status, message);
    }
    if (response.status === 429) {
      throw new GeminiError("Rate limited by the Gemini API.", 429, message);
    }
    throw new GeminiError(`Gemini API error (${response.status}): ${message}`, response.status, message);
  }
...
/**
 * Repairs unescaped single backslashes in JSON strings before LaTeX command names
 * so JSON.parse does not interpret \f as form-feed or \t as tab.
 */
export function repairJsonLatexEscapes(rawJson) {
  if (typeof rawJson !== "string") return rawJson;
  return rawJson.replace(
    /(?<!\\)(?:\\\\)*(\\)(?=(?:frac|dfrac|cfrac|text|tau|theta|times|to|rho|right|rightarrow|Rightarrow|beta|mathbf|begin|bar|bf|nabla|neq|nu|partial|Delta|alpha|gamma|sum|int|prod|infty|ext)\b)/g,
    (match) => match + "\\"
  );
}
...
export function readJson(payload) {
  ...
  try {
    return JSON.parse(repairJsonLatexEscapes(text));
  } catch {
    try {
