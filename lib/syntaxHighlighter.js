...
CORE_LANGUAGES.forEach((item) => {
  LANGUAGE_MAP.set(item.id, item.id);
  item.aliases.forEach((alias) => LANGUAGE_MAP.set(alias, item.id));
});

export function normalizeLanguage(lang) {
  if (!lang) return "javascript";
  const lower = String(lang).toLowerCase().trim();
  return LANGUAGE_MAP.get(lower) || "javascript";
}
