...
// B41: 30 days put the zinc anode's exhaustion out of reach. Zinc's ribbon
// lasts ~69 days in saltwater, so "anode used up -- rusting again" -- a
// modelled state with its own verdict string and its own HUD note -- could
// never be seen. Lowering `wrapMassG` was the alternative and would have been
// wrong: the wraps are the same ribbon in each metal, and 0.5 g zinc to
// 0.12 g magnesium is just their density ratio. So the run is longer instead.
...
export function solveCouple({ days = 7, electrolyte = "distilled", partner = "zinc" } = {}) {
  ...
  // The cell potential, which is a magnitude: E(cathode) - E(anode), and
  // for a spontaneous couple that is positive whichever way round the pair
  // sits. It used to be the signed `IRON_POTENTIAL_V - P.potential` under
  // the label "Driving voltage", so every consumer wrapped it in Math.abs
  // and the sign was never read. `direction` below carries the polarity.
  const deltaE = Math.abs(IRON_POTENTIAL_V - P.potential);
  ...
  // Once a sacrificial wrap is used up there is no couple left: the bare
  // nail is its own anode and cathode, and it simply rusts. The electrons
  // stop flowing between the metals, and the half-equations are iron's.
  const exhausted = partnerRemaining <= 0;
  const partnerIsAnode = P.potential < IRON_POTENTIAL_V;
  ...
  return {
    ...
    direction: exhausted ? "none" : partnerIsAnode ? "partner_to_iron" : "iron_to_partner",
    anode: exhausted || !partnerIsAnode ? "Fe" : P.symbol,
    cathode: exhausted ? "Fe" : partnerIsAnode ? "Fe" : P.symbol,
    iron,
    bare,
    ...
    oxidation: P.protects && !exhausted ? `${P.symbol} → ${ionSymbol(P.symbol, P.charge)} + ${electronsText(P.charge)}` : RUST_EQUATIONS.oxidation,
