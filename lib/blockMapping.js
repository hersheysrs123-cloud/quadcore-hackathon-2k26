 ...
 *      ways.
 *   2. The editor has `bullet`, `code` and `action`, which the original enum
...
const EDITOR_TO_LEVEL = { h1: 1, h2: 2, h3: 3 };
const LEVEL_TO_EDITOR = { 1: "h1", 2: "h2", 3: "h3" };
  ...
  switch (row?.block_type) {
    ...
    case "code":
      return {
        ...
        type: "code",
        content: content.text ?? "",
        meta: { language: content.language ?? null },
      };
    ...
    case "action":
      return {
        ...
        type: "action",
        content: content.text ?? "",
        actionKind: content.actionKind || content.action || "socratic",
        meta: { action: content.action ?? null, actionKind: content.actionKind ?? null },
      };
    ...
    // No editor equivalent — flatten to text and stash the rest on meta so
    // toDbBlock can rebuild the row exactly.
    case "media":
      return {
        id,
        type: "text",
...
export function toDbBlock(block) {
  ...
  switch (block?.type) {
    ...
    case "code":
      return row("code", { text, language: meta.language ?? null });

    case "action": {
      const act = meta.action ?? block.actionKind ?? meta.actionKind ?? "socratic";
      const payload = { text, action: act };
      if (meta.actionKind || (block.actionKind && block.actionKind !== act)) {
        payload.actionKind = block.actionKind ?? meta.actionKind;
      }
