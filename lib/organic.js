...
/**
 * Esterification: the n-carbon carboxylic acid + methanol ⇌ the methyl ester
 * the Ester series draws + water, warmed with a few drops of concentrated
 * sulfuric acid as the catalyst.
 *
 * CₙH₂ₙO₂ + CH₄O → Cₙ₊₁H₂ₙ₊₂O₂ + H₂O balances for every n: n + 1 carbons,
 * 2n + 4 hydrogens and 3 oxygens a side. The water is the acid's –OH plus
 * the alcohol's hydroxyl H. Isotope labelling shows it is that way round:
 * the ester's C–O–C oxygen came from the alcohol.
 */
export const ESTER_ALCOHOL_CARBONS = 1;

export function esterification(carbons) {
  const n = Math.max(1, Math.min(MAX_CARBONS, Math.round(carbons)));
  const acid = { formula: formulaFor("acid", n), name: nameFor("acid", n) };
  const alcohol = {
    formula: formulaFor("alcohol", ESTER_ALCOHOL_CARBONS),
    name: nameFor("alcohol", ESTER_ALCOHOL_CARBONS),
  };
  const ester = { formula: formulaFor("ester", n), name: nameFor("ester", n) };
  const water = { formula: "H₂O", name: "water" };
  return {
    acid,
    alcohol,
    ester,
    water,
    catalyst: "conc. H₂SO₄",
    equation: `${acid.formula} + ${alcohol.formula} ⇌ ${ester.formula} + ${water.formula}`,
    wordEquation: `${acid.name} + ${alcohol.name} ⇌ ${ester.name} + ${water.name}`,
  };
}
