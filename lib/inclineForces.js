...
export function advanceBlock(motion, options, dt) {
  ...
  // If already at an end stop and the net force continues driving the crate into the barrier,
  // the mechanical stop provides an equal reaction force, keeping the block at rest.
  if (position >= limit - 1e-4 && (velocity > 0 || solved.netForce > 0)) {
    return {
      position: limit,
      velocity: 0,
      arrivalVelocity: 0,
      hitBarrier: true,
      solved: { ...solved, netForce: 0, acceleration: 0, state: "static" },
    };
  }
  if (position <= -limit + 1e-4 && (velocity < 0 || solved.netForce < 0)) {
    return {
  ...
  if (velocity !== 0 && nextVelocity * velocity < 0) {
    ...
    if (atRest.state === "static") {
      return { position, velocity: 0, arrivalVelocity: 0, hitBarrier: false, solved: atRest };
    }
    nextVelocity = 0;
  }

  let nextPosition = position + nextVelocity * dt;
  let arrivalVelocity = nextVelocity;
  ...
  return { position: nextPosition, velocity: nextVelocity, arrivalVelocity, hitBarrier, solved };
}
