...
export function stepRun(state, track, { mass = 500, friction = true, g = G }, dt) {
  ...
  // A cart that has run out of energy on a rise rolls back down it.
  if (speed <= 1e-3) {
    const higherAhead = sampleAt(track, track.height, state.s + direction * 0.4) > height + 1e-6;
    if (higherAhead) direction = -direction;
    else stopped = true;
  } else {
    stopped = false;
  }

  const ds = direction * speed * dt;
  let s = state.s + ds;
