 ...
 *
 * NOTE: Every note in DEMO_NOTES contains EVERY SINGLE SUPPORTED BLOCK TYPE (except canvas):
 * h1, h2, h3, h4, text, bullet, number, todo, toggle, callout, quote,
 * divider, code, math, inlinemath, site, media!
 *
 * All 17 block types are naturally interwoven across each note's sections.
 */
  ...
  // ── Misc Space (Note 6: Quantum Mechanics — Wavefunctions & Atomic Orbitals) ────────────────
  {
    ...
    blocks: [
      ...
      b(
        ...
        "inlinemath",
        "$E_n = \\frac{n^2 \\pi^2 \\hbar^2}{2 m L^2} \\qquad \\text{for} \\quad n = 1, 2, 3, \\dots$"
      ),
      b(
      ...
      b(
        ...
        {
          open: false,
          details: "The quantum harmonic oscillator Hamiltonian can be factorized using dimensionless creation and annihilation ladder operators. The energy eigenvalues are equispaced, establishing a non-zero zero-point vacuum ground energy.",
        }
      ),
      b(
        "qua_divider",
...
export function demoNotesBySpace() {
  const bySpace = { School: [], Personal: [], Misc: [], Journal: [] };
  ...
  for (const note of DEMO_NOTES) {
    const copy = {
      ...
      fontStyle: note.fontStyle ?? "sans",
      fullWidth: Boolean(note.fullWidth ?? false),
      isLocked: Boolean(note.isLocked ?? false),
