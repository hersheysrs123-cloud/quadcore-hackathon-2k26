...
export function divisionPose(mode, t) {
  ...
  //
  // This has to hold on BOTH sides of the division boundary. Gated on
  // `division === 1` alone it described the end of meiosis I correctly and
  // then dropped to 0 the moment division 2 began, so the chromosomes popped
  // from 0.35 to fully decondensed between telophase I and prophase II -- the
  // one transition in meiosis where they are supposed to stay condensed.
  const restCondense =
    meiosis && ((division === 1 && after("anaphase")) || (division === 2 && !after("prophase")))
      ? 0.35
      : 0;
