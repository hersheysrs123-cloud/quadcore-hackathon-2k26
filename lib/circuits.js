...
/**
 * Bulbs still screwed in that have gone dark because a SERIES partner was
 * unscrewed — the ones a student would otherwise think were "unaffected".
 *
 * Opening one socket opens its whole branch, so a bulb sharing that branch
 * loses its only path: in the combination circuit, unscrewing A darkens B
 * too, and only C survives. (In a plain parallel circuit no branch holds two
 * bulbs, so the list is empty, and in a plain series circuit everything is
 * dead and the caller reports that instead.)
 */
export function strandedBulbs(solved) {
  const openBranches = new Set(solved.branches.filter((b) => b.open).map((b) => b.id));
  return solved.bulbs.filter((b) => !b.removed && openBranches.has(b.branch)).map((b) => b.id);
}
