 ...
 * Sanitizes and converts interactive / custom blocks into clean standard
 * structures (To-Do, Toggle flattening, Canvas badges, Bookmark links, Media links)
 * before passing to DOCX, HTML, Markdown, and Plain Text serializers.
 */
export function filterBlocksForExport(blocks = []) {
  ...
  for (const block of blocks) {
    ...
    switch (type) {
      case "todo":
        // Clean leading [ ] or [x] if present in raw content
      ...
      case "toggle":
        // Flatten toggle header + nested children into bullet list items
        sanitized.push({ id: block.id || makeBlockId(), type: "bullet", content: content || "Toggle Item" });
      ...
      case "drawing":
        sanitized.push({
          ...
          type: "text",
          content: `[Canvas Drawing: ${block.title || block.meta?.title || "Untitled Drawing"}]`,
        });
        break;

      case "bookmark":
      case "site_embed":
        const bmTitle = block.title || block.meta?.title || content || "Bookmark";
        const bmUrl = block.url || block.meta?.url || "#";
        sanitized.push({
          id: block.id || makeBlockId(),
          type: "text",
          content: `[${bmTitle}](${bmUrl})`,
        });
        break;

      case "media":
        const mediaUrl = block.url || block.meta?.url || content || "Media";
        sanitized.push({
          id: block.id || makeBlockId(),
          type: "text",
          content: `[Media Embed: ${mediaUrl}]`,
        });
        break;
...
export function exportMarkdown(blocks, title = "Note") {
  const filtered = filterBlocksForExport(blocks);
  const mdContent = blocksToMarkdownLossy(filtered);
  ...
  const contentHtml = (blocks || [])
    .map((block) => {
      ...
      switch (block.type) {
        ...
        case "code":
          return `<pre><code class="language-${block.language || block.meta?.language || "text"}">${content}</code></pre>`;
        case "math":
...
export function exportHtml(blocks, title = "Note", emoji = "📝") {
  const filtered = filterBlocksForExport(blocks);
  const htmlContent = blocksToHTMLLossy(filtered, title, emoji);
...
export function exportTxt(blocks, title = "Note") {
  const filtered = filterBlocksForExport(blocks);
  const txtContent = blocksToPlainText(filtered, title);
...
export async function blocksToDocxBlob(rawBlocks = [], title = "Untitled Note", emoji = "📝") {
  ...
  (blocks || []).forEach((block) => {
    ...
    switch (block.type) {
      ...
      case "code":
        children.push(new Paragraph({
          children: [new TextRun({ text: content, font: "Consolas" })],
          spacing: { before: 120, after: 120 },
          indent: { left: 360 },
        }));
        break;
      case "math":
        children.push(new Paragraph({
          children: [new TextRun({ text: `Formula: ${content}`, font: "Consolas", italic: true })],
          spacing: { before: 120, after: 120 },
          indent: { left: 360 },
        }));
        break;
...
export async function exportDocx(blocks, title = "Note", emoji = "📝") {
  const filtered = filterBlocksForExport(blocks);
  const blob = await blocksToDocxBlob(filtered, title, emoji);
