...
export function filterBlocksForExport(blocks = []) {
  ...
  for (const block of blocks) {
    ...
    switch (type) {
      ...
      // ── Canvas / Drawing: convert legacy drawing to callout block
      case "canvas":
      case "drawing": {
        const canvasTitle = content || block.title || block.meta?.title || "Drawing";
        sanitized.push({
          id: block.id || makeBlockId(),
          type: "callout",
          content: canvasTitle.startsWith("[Canvas]") ? canvasTitle : `[Canvas] – ${canvasTitle}`,
        });
        break;
      }
  ...
  return blocks
    .map((block) => {
      ...
      switch (block.type) {
        ...
        case "canvas": {
          const title = block.content || block.title || "Untitled Drawing";
          return `[Canvas] – ${title}`;
        }
        case "columns": {
          const count = Math.max(2, Math.min(5, Number(block.columnCount) || 2));
...
function blockToHtmlFragment(block, allBlocks = []) {
  ...
  switch (block.type) {
    ...
    case "canvas":
    case "drawing": {
      const canvasTitle = escapeHtml(block.content || block.title || block.meta?.title || "Drawing / Canvas");
      return `<div class="canvas-block" style="padding:12px 16px; background:#181c27; border:1px dashed #475569; border-radius:8px; margin:16px 0; color:#94a3b8; font-size:13px; display:flex; align-items:center; gap:10px;"><span style="font-size:18px;">🎨</span><span><strong style="color:#e2e8f0;">[Canvas Drawing]</strong> ${canvasTitle}</span></div>`;
    }
    case "columns": {
      const count = Math.max(2, Math.min(5, Number(block.columnCount) || 2));
      const cols = getNormalizedColumnsData(block.columnsData, block.content, count);
...
export function blocksToHTMLLossy(rawBlocks = [], title = "Untitled Note", emoji = "📝") {
  ...
  (blocks || []).forEach((block) => {
    if (block.type === "bullet" || block.type === "number") {
      ...
      const styleAttr = lvl > 0 ? ` style="margin-left: ${lvl * 1.5}em; list-style-type: ${lvl === 1 ? 'circle' : 'square'};"` : "";
...
export function blocksToPlainText(rawBlocks = [], title = "") {
  ...
  (blocks || []).forEach((block) => {
    ...
    switch (block.type) {
      ...
      case "canvas":
      case "drawing": {
        const title = block.content || block.title || "Drawing";
...
export async function blocksToDocxBlob(rawBlocks = [], title = "Untitled Note", emoji = "📝") {
  ...
  (blocks || []).forEach((block) => {
    ...
    switch (block.type) {
      ...
      case "canvas":
      case "drawing": {
        const canvasTitle = block.content || block.title || "Drawing / Canvas";
        children.push(new Paragraph({
          children: [
            new TextRun({ text: "🎨 [Canvas Drawing]: ", bold: true, color: "6366F1" }),
            new TextRun({ text: canvasTitle, italic: true }),
          ],
          indent: { left: 360 },
          spacing: { before: 80, after: 80 },
        }));
