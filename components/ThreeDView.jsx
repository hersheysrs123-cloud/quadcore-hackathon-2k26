...
export default function ThreeDView({ hideTopBars = false, onToggleTopBars, onStudyTopic }) {
  ...
  const handleOpenStudy = useCallback(
    (targetTopic, targetParams) => {
      if (isCustomSelected && activeWidgetObj) {
        if (onStudyTopic) {
          onStudyTopic(
            {
              concept: activeWidgetObj.title || "3D Interactive Model",
              focus: activeWidgetObj.title || "",
              content: `# ${activeWidgetObj.title}\n\nCustom 3D Interactive Model generated by Socratic Duck.\n\nJSON Schema:\n\`\`\`json\n${JSON.stringify(activeWidgetObj.widget, null, 2)}\n\`\`\``,
              noteId: `3d_custom_${activeWidgetObj.id}`,
              noteTitle: `3D: ${activeWidgetObj.title}`,
              space: "Duck Models",
            },
            "explain",
          );
    ...
    [isCustomSelected, activeWidgetObj, topic, params, onStudyTopic],
  );
