  ...
  return (
    <group rotation={shellTilt(index)}>
      {showRing && (
        <Line
          points={ring}
          color={glow ? PALETTE.goldDim : PALETTE.line}
          lineWidth={glow ? 1.8 : 1}
          transparent
          opacity={dimmed ? 0.1 : glow ? 0.75 : 0.5}
          onClick={(e) => {
            e.stopPropagation();
            onSelect(index);
          }}
...
const ALKANE_NAMES = ["methane", "ethane", "propane", "butane"];
const ALKENE_NAMES = ["—", "ethene", "propene", "butene"];
const ALCOHOL_NAMES = ["methanol", "ethanol", "propanol", "butanol"];
...
const CC_BOND = 1.54; // C–C, in the same units the rest of the scene uses
const CC_DOUBLE = 1.34; // C=C is shorter, as a second shared pair pulls it in
const CH_BOND = 1.09;
 ...
 * placed at the directions VSEPR actually predicts — which for a –CH₂– means
 * out of the plane of the chain, not flat in it.
 */
function buildMolecule(family, carbons) {
  const n = clamp(carbons, 1, 4);
  ...
  for (let i = 0; i < n; i += 1) {
    if (i > 0) {
      const L = isAlkeneChain && i === 1 ? CC_DOUBLE : CC_BOND;
      x += L * CHAIN_X;
    }
    chain.push(new THREE.Vector3(x, i % 2 === 0 ? rise : -rise, 0));
  }
  // Centre the finished chain rather than guessing its width up front.
  ...
  for (let i = 0; i < n - 1; i += 1) {
    bonds.push({ from: chain[i].toArray(), to: chain[i + 1].toArray(), double: isAlkene && i === 0 });
  }

  // Oxygen of the –OH group hangs off carbon 1.
  let oxygen = null;
  if (isAlcohol) {
  ...
  for (let i = 0; i < n; i += 1) {
    ...
    const used = neighbours.length + (doubleHere ? 1 : 0);
    ...
    // Which plane the remaining bonds open into is the part that was wrong.
    // A carbon with two neighbours puts its hydrogens *perpendicular* to the
    // plane those two bonds define; a carbon carrying a double bond keeps
    // everything in that plane, because sp² is flat.
    let side;
    if (doubleHere) {
      // Stay in the molecular plane: 120° apart, trigonal planar.
  ...
  const valid = !(family === "alkene" && n < 2);
  const hCount = atoms.filter((a) => a.el === "H").length;
  // The –OH hydrogen is written with the oxygen, not counted into the chain.
  const formula = !valid
    ? "—"
    : isAlcohol
      ? `C${sub(n)}H${sub(hCount - 1)}OH`
      : `C${sub(n)}H${sub(hCount)}`;
  const name = !valid
    ? "needs 2+ carbons for a C=C"
    : isAlkene
      ? ALKENE_NAMES[n - 1]
      : isAlcohol
        ? ALCOHOL_NAMES[n - 1]
...
export function OrganicBuilderScene({ params }) {
  ...
  return (
    <SceneCanvas camera={{ position: [0, 2.4, 8.5], fov: 45 }}>
      ...
      <SceneReadout
        ...
        rows={[
          ...
          [
            "General formula",
            family === "alkane" ? "CₙH₂ₙ₊₂" : family === "alkene" ? "CₙH₂ₙ" : "CₙH₂ₙ₊₁OH",
          ],
          ["Carbons n", carbons],
          ["Bonds per carbon", "4"],
          ["Saturated", family === "alkene" ? "no — has C=C" : "yes", family === "alkene" ? "bad" : "good"],
          ["Bromine water", family === "alkene" ? "decolourised" : "stays orange"],
        ]}
        note={
          !molecule.valid
            ? "An alkene needs a C=C double bond, so it must have at least two carbons. Add one."
            : crackable
              ? "Press “Trigger cracking”: this alkane breaks into a shorter alkane plus a useful alkene, which is how a refinery matches supply to demand."
              : family === "alkene"
                ? "The C=C double bond is what decolourises bromine water — the standard test for unsaturation."
                : "Each member of the series differs by CH₂, so the properties change gradually down the series."
        }
      ...
      <SceneLegend
        ...
        items={[
          ...
          { color: ATOM_STYLE.O.color, label: "Oxygen", note: "two bonds — here as the –OH group" },
          {
            color: family === "alkene" ? PALETTE.gold : "#3f4854",
            shape: "line",
            label: family === "alkene" ? "C=C double bond" : "Single C–C bond",
            note:
              family === "alkene"
                ? "two shared pairs — the reactive site"
                : "one shared pair — saturated and unreactive",
          },
        ]}
      />
    </SceneCanvas>
...
export function CrystalLatticeScene({ params }) {
  ...
  return (
    <SceneCanvas camera={{ position: [6, 4.5, 8], fov: 45 }}>
      ...
      <SceneReadout
        title={facts.title}
        subtitle={structure === "nacl" ? "Giant ionic lattice" : "Giant covalent lattice"}
        rows={facts.rows}
        note={facts.note}
      />
  ...
  return (
    <SceneCanvas camera={{ position: [0, 3, 10], fov: 45 }}>
      ...
      <mesh position={[-TANK.w / 2 + 1, 0.3, 0]}>
        ...
        <meshStandardMaterial color="#b45309" emissive="#b45309" emissiveIntensity={0.4} metalness={0.7} roughness={0.35} />
      </mesh>
      {/* Anode (+) on the right, dissolving away as the cathode thickens. */}
      <mesh position={[TANK.w / 2 - 1, 0.3, 0]}>
        <cylinderGeometry args={[anodeRadius, anodeRadius, TANK.h + 1.4, 24]} />
        <meshStandardMaterial color="#7c3f12" emissive={PALETTE.gold} emissiveIntensity={0.25} metalness={0.7} roughness={0.4} />
      </mesh>
