...
│       ├── lab-bench.jsx                 # Shared lab-bench kit: cm() scale, GLASS/PORCELAIN/STEEL/BRASS presets, LabBench, LabWall (tiled wall + shelves, useTileTexture), HeatMat, RetortStand (ring/clamp fittings), Tripod, BunsenBurner (ref-driven flame: two per-frame lathe shells + glow sprite, useGlowTexture), LiquidStack, ConicalFlask, Funnel, EvaporatingBasin, GlassJar, HeatShield, Tongs, SprayBottle
