...
const TOPICS = [
  ...
  },
  {
    ...
    blurb: "Ball-and-stick alkanes, alkenes and alcohols",
    syllabus: "Chemistry 14 · Organic chemistry",
    keywords: "organic alkane alkene alcohol homologous series cracking saturated unsaturated bromine ethanol methane",
    ...
    controls: [
      ...
      { type: "slider", key: "carbons", label: "Carbon chain length", min: 1, max: 4, step: 1, format: (v) => `C${v}` },
  ...
  },
  {
    ...
    blurb: "NaCl ionic cube, diamond network, graphite sheets",
    syllabus: "Chemistry 3 · Structure & bonding",
    keywords: "lattice giant ionic covalent nacl sodium chloride diamond graphite allotrope delocalised conductivity",
  ...
  return (
    <div className="flex min-h-screen flex-col bg-ink-950 lg:h-screen lg:overflow-hidden">
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-3 px-5 py-3.5">
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-[11px] text-ink-500">
              IGCSE Grade 10 · {TOPICS.length} interactive 3D models
            </p>
          </div>
                ...
                <button
                  ...
                  className={`flex items-center gap-1.5 rounded-md border px-2.5 py-1.5 text-xs transition-colors ${
                    active
                      ...
                      : "border-transparent text-ink-400 hover:bg-ink-800 hover:text-ink-200"
                  }`}
                >
      ...
      <div className="flex min-h-0 flex-1 flex-col lg:flex-row">
        {/* Sidebar list */}
        <aside className="w-full shrink-0 border-b border-ink-800 bg-ink-900 lg:w-[300px] lg:overflow-y-auto lg:border-b-0 lg:border-r">
          <p className="px-4 pb-2 pt-4 text-[11px] font-medium uppercase tracking-wider text-ink-500">
            {visibleTopics.length} visualization{visibleTopics.length === 1 ? "" : "s"}
          </p>

          {visibleTopics.length === 0 ? (
            <p className="px-4 pb-4 text-xs leading-relaxed text-ink-500">
              Nothing matches “{query}”. Try a syllabus term like <em>osmosis</em>,{" "}
              <em>cracking</em> or <em>critical angle</em>.
            </p>
          ) : (
                ...
                return (
                  <li key={t.id}>
                    <button
                      ...
                      suppressHydrationWarning
                      className={`flex w-full gap-2.5 rounded-lg border px-2.5 py-2.5 text-left transition-colors ${
                        active
                          ? "border-duck-500/50 bg-duck-500/10"
                          : "border-transparent hover:border-ink-700 hover:bg-ink-850"
                      }`}
                    >
                      <span
                        className={`mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-md ${
                          active ? "bg-duck-500/20" : "bg-ink-800"
                        }`}
                      >
                        <Icon
                          className={`h-3.5 w-3.5 ${active ? "text-duck-300" : "text-ink-400"}`}
                          strokeWidth={2}
                        />
                      </span>
                      <span className="min-w-0 flex-1">
                        ...
                        <span className="mt-1 inline-block rounded border border-ink-700 px-1.5 py-0.5 text-[10px] text-ink-500">
                          {t.syllabus}
                        </span>
                      </span>
                    </button>
