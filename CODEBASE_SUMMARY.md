...
- **Notion-Grade Line Splitting & Backspace Merging (`splitBlockDOMAtRange` & `getDOMCaretLength`)**:
  - **Heading Enter Prepend (`onAddBefore`)**: Pressing `Enter` at offset 0 (start) of any heading block (`h1`–`h4`) prepends a new blank text paragraph immediately above the heading and shifts the heading and all blocks below downward, maintaining caret focus on the heading (matching Notion).
