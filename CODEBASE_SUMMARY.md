...
22. **Incline Plane & Static/Kinetic Friction Apparatus Architecture (`InclineFrictionCanvas.jsx`, `force-diagram.jsx`, `inclineForces.js`)**:
    ...
    - **Mathematical Zero-Axes in `GraphPanel`**: Replaced hardcoded bottom-L axes with dynamic `xZero` and `yZero` mappings. Bipolar plots render the true $v = 0$ baseline through the center of the graph while bounding the plot perimeter with a subtle, low-opacity frame (`lineWidth: 1.2`, opacity $0.45$), eliminating confusing bright floor lines.
