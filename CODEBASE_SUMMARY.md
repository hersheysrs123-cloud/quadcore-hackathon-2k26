...
21. **Projectile Motion & Air Resistance Kinematics and 60 FPS Vector Architecture (`PhysicsCanvas.jsx`)**:
    - **Collinear Cannon Muzzle & Elevated Runway Clearance**: Launch origin is pinned at $[0, \text{LAUNCH\_Y}, 0]$ where $\text{LAUNCH\_Y} = RUNWAY\_TOP\_Y + BALL\_RADIUS = 0.41$ ($RUNWAY\_TOP\_Y = 0.28$, $BALL\_RADIUS = 0.13$). The runway begins strictly at $X = 0$, while the cannon carriage base is seated at $X \le 0$ ($X = -0.52$ to $0$), guaranteeing 0% clipping between the launcher, floor, and runway. Compact barrel length ($0.46\text{ m}$) guarantees the breech never dips below the floor across elevation angles ($5^\circ$–$85^\circ$). Landing target bullseyes sit at $Y = 0.282$, meeting the bottom of the landing projectile ($0.41 - 0.13 = 0.28$) with exact mathematical precision.
    - **Lightened Scientific Instrument Palette**: Upgraded runway to a light porcelain slate bed (`#f1f5f9`) with silver deck (`#e2e8f0`), satin aluminum curbs (`#cbd5e1`), and dark graphite graduation ticks (`#475569`). Cannon upgraded to ultra-light polished platinum (`#f8fafc`), champagne brass muzzle crown & bands (`#fde047`), mirror chrome cascabel & trunnions (`#ffffff`), and white protractor quadrant (`#ffffff`).
