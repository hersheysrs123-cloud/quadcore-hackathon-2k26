...
│   ├── QuizPanel.jsx                     # Study sidebar drawer: Dynamic AI diagnostic quiz generator, assessment runner & mastery tracker
...
- **19 Block Types Supported**:
  ...
  6. `bullet`: Unordered list item with auto-continuation, hierarchical multi-tier indent levels (Level 0 `●`, Level 1 `○`, Level 2 `■`, Level 3 `□`), `Tab`/`Shift+Tab` indentation, Backspace un-indentation & conversion to text with empty DOM fallback, and escape on empty Enter.
  7. `number`: Ordered list item with dynamic sequential & hierarchical counting (Level 0 `1.`, `2.`, `3.`; Level 1 `a.`, `b.`, `c.`; Level 2 `i.`, `ii.`, `iii.`; Level 3 `A.`, `B.`, `C.`), `Tab` / `Shift+Tab` indentation (replacing soft-tab whitespace insertion), level inheritance on Enter, hierarchical reset across block boundaries, and Backspace un-indentation.
...
- **Floating Text Selection Popover Toolbar (`TextSelectionToolbar`)**:
  ...
  - **Undo/Redo & Multi-Block Selection Focus Retention (`BUG-UNDO-01`, `BUG-SEL-01` & Section 78)**: Global undo/redo and deleting multi-block marquee selections seamlessly retains focus on the target or adjacent remaining block instead of dropping focus to `document.body` or jumping to block 0. Employs `selectedIdRef` and intelligent neighboring block resolution when an undone action removes the previously selected block.
  ...
  - **Centralized Deeply-Cloned Undo/Redo Engine (`pushHistorySnapshot`)**:
    ...
    - Symmetrical smart target focus restoration across both Undo and Redo.
...
- **Quiz Drawer Assessment Engine (`components/QuizPanel.jsx`)**: Dedicated quiz assessment sidebar for active notes and selections, evaluating understanding through dynamic questions and recording session scores directly into the mastery analytics store. (The legacy conversational Socratic Duck chatbot tab was streamlined out of the quiz drawer to prioritize focused testing and assessment).
